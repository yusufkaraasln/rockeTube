...
function CompanyUpdate({ editable }) {
  ...
  const uploadFile = (file) => {
    ...
    uploadTask.on(
      ...
      () => {
        if (currentData.logo) {
          const decoded = decodeURI(currentData.logo);
          const deletedFile = decoded?.split("%2F")[1]?.split("?")[0];
          const desertRef = ref(storage, "/company/" + deletedFile);
        ...
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          setCurrentData((prev) => ({ ...prev, logo: downloadURL }));
        });
      }
    );
  };
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(
        `http://localhost:5000/api/company/update/${editable.compId}`,
        currentData
      );
      showToastMessage("success", "Company");
      setInterval(() => {
      ...
      <div className="company_update">
        ...
        <div className="company_update__logo">
          <div className="update_logo_wrapper">
            <img src={currentData.logo ? currentData.logo : NoPerson} alt="" />
