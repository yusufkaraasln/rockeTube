...
import UploadActor from "./UploadActor";
import ActorUpdate from "../../updates/actor/ActorUpdate";

function Actors() {
  const [isLoaded, setIsLoaded] = useState(true);
  const handleUpdate = (actorId) => {
    setUpdateModal({
      actorId,
      actorData: data.find((item) => item._id == actorId),
      modal: true,
    });
  };
  ...
  const columns = [
...

    {
      ...
      width: 300,
      renderCell: (params) => {
        return params.row.films?.map((film) => {
          return (
            <div
              style={{
                color: "white",
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Silmek istediğinize emin misiniz?")) {
      await axios.delete("http://localhost:5000/api/actor/delete/" + id);
      ...
      <>
        ...
        {videoModal ? (
          ...
          <div className="videos_container">
            {updateModal.modal ? (
              <ActorUpdate editable={updateModal} />
            ) : (
              <>
                {isLoaded ? (
                  <i
